...
fn transform_coeffs(
    ...
    // Never zero. Processing indexes the segment ring with `% nsegments` and
    // cannot report an error, so a filter built with no coefficients has to
    // come out as one silent segment rather than a division by zero. A
    // configuration cannot reach this, `validate_config` rejecting empty
    // coefficients, but the constructors are public.
...
mod tests {
    ...
    /// first chunk, dividing by a segment count of zero. Processing cannot
    /// report an error, so this has to be caught earlier and be harmless if it
    /// somehow is not.
    #[test]
